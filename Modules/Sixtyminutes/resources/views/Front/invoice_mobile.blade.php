...
<body>
    ...
    <div class="logo-wrap">
        <img src="{{ asset('resources/' . env('TEMPLATE_NUM', '') . '/assets/images/logo.png') }}" alt="Logo">
    </div>
    ...
    <div class="invoice-card">
        ...
        <div class="invoice-body">
            <div class="invoice-row">
                ...
                <span class="value">{{ @$invoice->member->name }}</span>
            </div>
            ...
            <div class="invoice-row">
                ...
                <span class="value" style="color: green;">-{{ $invoice->discount_value }} {{trans('front.app_currency')}}</span>
            </div>
            @endif
            <hr class="divider">
            ...
            <div class="invoice-row">
                ...
                <span class="value">{{ number_format($invoice->amount_paid + @$invoice->amount_remaining - @$invoice->vat, 2) }} {{trans('front.app_currency')}}</span>
            </div>
            <div class="invoice-row">
                <span class="label">{{trans('front.vat')}} ({{ @$mainSettings->vat_details['vat_percentage'] }}%)</span>
                <span class="value">{{ number_format(@$invoice->vat, 2) }} {{trans('front.app_currency')}}</span>
            </div>
            @endif
            <div class="invoice-row total-row">
                <span class="label">{{trans('global.total')}}</span>
                <span class="value">{{ number_format($invoice->amount_paid + @$invoice->amount_remaining, 2) }} {{trans('front.app_currency')}}</span>
            </div>
        </div>
    </div>
    ...
    @if($qr_img_invoice)
    <div class="qr-wrap">
        <img src="{{ asset($qr_img_invoice) }}" alt="QR">
    </div>
    @endif
