...
@section('list_add_button')
    <a href="{{route('createCity')}}" class="btn btn-lg btn-success">{{trans('admin.add')}} City</a>
     @if(request('trashed'))
            <a href="{{route('listCity')}}" class="btn btn-lg btn-info">{{trans('admin.enabled')}}</a>
        @else
            <a href="{{route('listCity')}}?trashed=1" class="btn btn-lg btn-danger">{{trans('admin.disabled')}}</a>
        @endif
    ...
    <table class="table table-striped table-bordered table-hover" >
        ...
        @foreach($cities as $key=> $city)
            <tr>
                ...
                <td>
                    <a href="{{route('editCity',$city->id)}}" class="btn btn-xs yellow">
                        <i class="fa fa-edit"></i>
                    </a>
                    @if(request('trashed'))
                        <a title="{{trans('admin.enable')}}" href="{{route('deleteCity',$city->id)}}" class="confirm_delete btn btn-xs green">
                            <i class="fa fa-check-circle"></i>
                        </a>
                    @else
                        <a title="{{trans('admin.disable')}}" href="{{route('deleteCity',$city->id)}}" class="confirm_delete btn btn-xs red">
     ...
     <div class="col-lg-5 col-md-5 col-md-offset-5">
                {!! $cities->appends($search_query)->render()  !!}
            </div>
        </div>
@endsection
