                ...
                <div class="col-lg-8">
                    <div class="blog-box">
                        <div class="blog-content">
                            <h4>{{$record['name']}} <span style="color: #f97d04;float: {{ $lang == 'ar' ? 'left' : 'right' }};font-size: 16px;padding: 10px;background-color: #6c757d26;border-radius: 5px">{{trans('front.price')}}: {{number_format(($record['price'] + ($record['price'] * (@$mainSettings['vat_details']['vat_percentage']/100))), 2)}} {{trans('front.pound_unit')}}</span></h4>
{{--                            <p>It is a long established fact that a reader </p>--}}
                            ...
                            <form method="post" action="{{route('subscription-submit', @$record->id)}}">
                                {{csrf_field()}}
                                <input type="hidden" name="subscription_id" value="{{$record['id']}}">
                                <input type="hidden" name="amount" value="{{round($record['price'] + ($record['price'] * (@$mainSettings['vat_details']['vat_percentage']/100)))}}">
...
@section('script')
    <script>
        // Paymob payment - no additional client-side script needed
    </script>
@endsection
