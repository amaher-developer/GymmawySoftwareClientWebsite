...
Route::get('/payments/verify/{payment?}',[\App\Modules\Almadagym\app\Http\Controllers\Front\SubscriptionFrontController::class,'payment_verify'])->name('verify-payment');
Route::post('/payments/verify/{payment?}',[\App\Modules\Almadagym\app\Http\Controllers\Front\SubscriptionFrontController::class,'payment_verify'])->name('verify-payment');
...
Route::name('noJs')->get('noJs', [\App\Modules\Almadagym\app\Http\Controllers\Admin\DashboardAdminController::class, 'noJs']);

Route::name('backupDB')->get('operate/db-backup', [\App\Modules\Almadagym\app\Http\Controllers\Admin\DashboardAdminController::class, 'backupDb'])->middleware(['permission:super|dashboard']);

Route::name('uploadImageForCKEditorAjax')->post('upload-ckeditor-ajax', [\App\Modules\Almadagym\app\Http\Controllers\Admin\GenericAdminController::class, 'uploadImageForCKEditorAjax']) ->middleware(['auth']);

Route::name('siteOff')
    ->get('site-off', [\App\Modules\Almadagym\app\Http\Controllers\Front\MainFrontController::class, 'site_off'])->middleware(['auth:sw', 'auth']);
Route::name('siteOn')
    ->get('site-on', [\App\Modules\Almadagym\app\Http\Controllers\Front\MainFrontController::class, 'site_on'])->middleware(['auth:sw', 'auth']);
