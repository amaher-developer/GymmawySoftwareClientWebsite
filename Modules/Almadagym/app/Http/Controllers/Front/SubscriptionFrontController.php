...
use App\Modules\Almadagym\app\Models\MoneyBoxBalance;
...
//        $record = (array)$this->getSubscription($id, @$this->mainSettings['subscription']);
        ...
        $title = $record['name'];
        return view('almadagym::Front.subscription', compact('title', 'record', 'subscriptions'));
    }
...
//        $record = (array)$this->getSubscription($id, @$this->mainSettings['subscription']);
        ...
        $title = $record['title'];
        return view('almadagym::Front.pt_class', compact('title', 'record','pt_classes'));
    }
...
//        $record = (array)$this->getSubscription($id, @$this->mainSettings['subscription']);
        ...
        $subscriptions = @Subscription::where('is_web', true)->get();
        $title = $record['name'];
        return view('almadagym::Front.subscription_test', compact('title', 'record', 'subscriptions'));
    }
    ...
    protected function createMoneyBoxEntry(PaymentOnlineInvoice $invoice, $member, int $type): void
    {
        ...
        DB::transaction(function () use ($invoice, $member, $type, $branchSettingId, $notes) {
            ...
            MoneyBox::create([
                ...
                'notes' => $notes,
                'member_id' => is_array($member) ? $member['id'] : $member->id,
                'type' => $type,
                'payment_type' => Constants::ONLINE_PAYMENT,
