...
use Modules\Fitdose\app\Http\Controllers\Front\AuthFrontController;
use Modules\Fitdose\app\Http\Controllers\Front\GenericFrontController;
use Modules\Fitdose\app\Http\Classes\TabbyService;
use Modules\Fitdose\app\Http\Classes\TamaraService;
use Modules\Fitdose\app\Http\Classes\PaytabsService;
use Modules\Fitdose\app\Http\Requests\SubscriptionRequest;
use Modules\Fitdose\Models\Member;

use Modules\Fitdose\Models\MemberSubscription;
use Modules\Fitdose\Models\MoneyBox;
use Modules\Fitdose\Models\MoneyBoxBalance;
...
//        $record = (array)$this->getSubscription($id, @$this->mainSettings['subscription']);
        ...
        $title = $record['name'];
        return view('fitdose::Front.subscription', compact('title', 'record', 'subscriptions'));
    }
    ...
    public function showMobile($id)
    {
        ...
        $title = $record['name'];
        return view('fitdose::Front.subscription_mobile', compact('title', 'record'));
    }
    ...
    public function invoice($invoice_id)
    {
        ...
        return view('fitdose::Front.invoice', compact('title', 'invoice', 'qr_img_invoice'));
    }
    ...
    public function invoiceMobile($invoice_id)
    {
        ...
        $qr_img_invoice = @$invoice->qr_code;
        return view('fitdose::Front.invoice_mobile', compact('title', 'invoice', 'qr_img_invoice'));
    }
...
//                }
    ...
    public function payment_verify(Request $request)
    {
        ...
        try {
            ...
            DB::transaction(function () use ($payment, $payment_invoice, $selectedActivities, &$member_subscription) {
                ...
                if(!@$this->current_user->id){
                    ...
                    $branchSettingId = $payment_invoice->subscription->branch_setting_id ?? 1;
...
//        $id = $payment->payment->id;
    ...
    public function tabbyNotify(Request $request)
    {
        ...
        try {
            ...
            if (!$member) {
                $branchSettingId = $paymentInvoice->subscription->branch_setting_id ?? 1;
                $maxId = str_pad((Member::withTrashed()->where('branch_setting_id', $branchSettingId)->max('code') + 1), 14, 0, STR_PAD_LEFT);
    ...
    public function error_payment(){
        ...
        $title = trans('front.invoice');
        return view('fitdose::Front.error', compact('title'));
    }
    public function tabbyFailure(Request $request){
        ...
        $title = trans('front.invoice');
        return view('fitdose::Front.tabby_error_failure', compact('title'));
    }
    public function tabbyCancel(Request $request){
        ...
        $title = trans('front.invoice');
        return view('fitdose::Front.tabby_error_cancel', compact('title'));
    }
    ...
    public function tamaraFailure(){
        ...
        $title = trans('front.invoice');
        return view('fitdose::Front.tamara_error_failure', compact('title'));
    }
    ...
    public function tamaraCancel(){
        ...
        $title = trans('front.invoice');
        return view('fitdose::Front.tamara_error_cancel', compact('title'));
    }
    ...
    protected function finalizeTabbyCheckout(PaymentOnlineInvoice $invoice, string $joiningDate, $sessionMember = null, bool $loginNewMember = true): ?MemberSubscription
    {
        ...
        $result = DB::transaction(function () use ($invoice, $joiningDate, $sessionMember, $subscription) {
            ...
            if (!$member) {
                $generatedCode = str_pad(((int)Member::withTrashed()->where('branch_setting_id', $subscription->branch_setting_id ?? 1)->max('code') + 1), 14, 0, STR_PAD_LEFT);
                $member = Member::create([
                    'code' => $generatedCode,
                    'name' => $invoice->name,
                    ...
                    'dob' => $invoice->dob,
                    'branch_setting_id' => $subscription->branch_setting_id ?? 1,
                ]);
    ...
    protected function sendSubscriptionNotification(int $memberSubscriptionId, string $phone, int $type): void
    {
        try {
            ...
            Http::timeout(10)->post('https://fitdose.gymmawy.com/api/send-subscription-notification', [
                'lang'                   => 'ar',
    ...
    {
        $branchSettingId = $invoice->subscription->branch_setting_id ?? 1;
    ...
    public function paytabsCancel(Request $request)
    {
        ...
        $title = trans('front.invoice');
        return view('fitdose::Front.paytabs_error_cancel', compact('title'));
    }
    ...
    public function paytabsFailure(Request $request)
    {
        ...
        $title = trans('front.invoice');
        return view('fitdose::Front.paytabs_error_failure', compact('title'));
    }
