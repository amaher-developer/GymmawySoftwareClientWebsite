...
use Modules\Zonegym\Http\Classes\TabbyService;
use Modules\Zonegym\Http\Requests\SubscriptionRequest;
use Modules\Zonegym\Models\Member;

use Modules\Zonegym\Models\MemberSubscription;
use Modules\Zonegym\Models\MoneyBox;
use Modules\Zonegym\Models\MoneyBoxBalance;
use Modules\Zonegym\Models\PaymentOnlineInvoice;
...
//        $record = (array)$this->getSubscription($id, @$this->mainSettings['subscription']);
        ...
        $title = $record['name'];
        return view('zonegym::Front.subscription', compact('title', 'record', 'subscriptions'));
    }
...
//        $record = (array)$this->getSubscription($id, @$this->mainSettings['subscription']);
        ...
        $subscriptions = @Subscription::where('is_web', true)->get();
        $title = $record['name'];
        return view('zonegym::Front.subscription_test', compact('title', 'record', 'subscriptions'));
    }
    ...
    public function invoice($invoice_id)
    {
        ...
        return view('zonegym::Front.invoice', compact('title', 'invoice', 'qr_img_invoice'));
    }
...
//        $id = $payment->payment->id;
    ...
    public function error_payment(){
        $title = trans('front.invoice');
        return view('zonegym::Front.error', compact('title'));
    }
    public function tabbyFailure(){
        $title = trans('front.invoice');
        return view('zonegym::Front.tabby_error_failure', compact('title'));
    }
    public function tabbyCancel(){
        $title = trans('front.invoice');
        return view('zonegym::Front.tabby_error_cancel', compact('title'));
    }
    ...
    {
        $branchSettingId = 1; // Zonegym is single-branch; matches the DB column's default.

        $memberName = is_array($member) ? ($member['name'] ?? null) : $member->name;
        ...
        DB::transaction(function () use ($invoice, $memberName, $memberId, $type, $branchSettingId, $notes) {
