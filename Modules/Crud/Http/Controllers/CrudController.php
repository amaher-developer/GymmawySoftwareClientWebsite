...
class CrudController extends GenericAdminController
{
    ...
    public function index()
    {
        ...
        foreach ($modules as $key=> $module) {
            $moduleArray = ['slug' => $module->getName(), 'name' => $module->getName()];
            $moduleArray['migrate'] = false;
            foreach (File::allFiles(module_path($module->getName()) . '/Database/Migrations/') as $module_migrations) {
                $migration_name = pathinfo($module_migrations->getPathname())['filename'];
                if (sizeof(Migration::where('migration', $migration_name)->get()) == 0) {
                    $moduleArray['migrate'] = true;
                }
            }
            $modules[$key] = $moduleArray;
        }
    ...
    {
        $module['slug'] = Str::slug(Str::singular($request->input('name')));
        if (Module::find($module['slug']) === null) {
    ...
    public function show($slug)
    {
        ...
        $crud = (object)['slug' => $module->getName(), 'name' => $module->getName()];
        $migrate = false;
        $rollback = false;
...
//        $name = studly_case($name);
    ...
    private function create_controller_template($main_module_name, $model_name, $Folder)
    {
        ...
        $ControllerTemplate = $this->replace_template($ControllerTemplate, array(
            ...
            '{{plural_model_name}}' => Str::plural(strtolower($model_name)),
            '{{ucf_plural_model_name}}' => ucfirst(Str::plural(strtolower($model_name))),
            '{{list_route_name}}' => 'list' . ucfirst(Str::plural(strtolower($model_name)))
        ));
    ...
    public function addRequest(Request $request, $slug)
    {
        ...
        $name = Str::studly(Str::singular($request->input('name')));
...
//Create Model
    ...
    private function create_model_template($main_module_name, $model_name)
    {
        ...
        $studly_model = Str::studly($model_name);
        $appends_attribute = '';
        $appends_functions = '';
        ...
        foreach ($pure_fields as $key => $pure_field) {
            if (@$multi_language[$key]) {
                ...
                $appends_functions .=
                    '    public function get' . Str::studly($pure_field) . 'Attribute()
    {
...
//                $appends_attribute .= "'$pure_field'" . ',';
                ...
                $appends_functions .=
                    '    public function get' . Str::studly($pure_field) . 'Attribute($' . $pure_field . ')
    {
        if($' . $pure_field . ')
        {
        ...
        public function get' . Str::studly($pure_field) . 'ThumbnailAttribute()
    {
        if ($this->' . $pure_field . ') {
...
';
        ...
        $AdminControllerTemplate = $this->replace_template($AdminControllerTemplate, array(
            ...
            '{{lc_plural_model_name}}' => strtolower(Str::plural($studly_model)),
            '{{ucf_plural_model_name}}' => ucfirst(strtolower(Str::plural($studly_model))),
    ...
    public function addSubModule(Request $request, $slug)
    {
        ...
        $name = $request->input('name');
        $table_name = Str::plural(strtolower(str_replace(' ', '_', $name)));
        $controller_name = $name = Str::studly(Str::singular($name));
        $sections = ['1' => 'Admin', '2' => 'Api', '3' => 'Front'];
...
//        $null_able = request()->get('nullable');
    ...
    private function create_admin_controller_template($main_module_name, $model_name, $adminFolder,$slug)
    {
        ...
        $AdminControllerTemplate = $this->replace_template($AdminControllerTemplate, array(
            ...
            '{{lcf_module_name}}' => strtolower($main_module_name),
            '{{lcf_model_name}}' => strtolower($model_name),
            '{{ucf_model_name}}' => Str::studly($model_name),
    ...
    private function create_factory_template($main_module_name, $model_name, $factory_inputs,$slug)
    {
        ...
        $modelClass = 'App\\Modules\\' . $main_module_name . '\\Models\\' . $model_name;
        $DatabaseSeederNewContent = '
        /*$factory*/
        ' . $modelClass . '::factory()->count(50)->create();
        ';
...
//    }
    ...
    private function create_admin_form_template($slug, $controller_name)
    {
        ...
        $AdminRoutesTemplate = $this->replace_template($AdminRoutesTemplate, array(
            ...
            '{{plural_model_name}}' => strtolower(Str::plural($controller_name)),
            '{{ucf_plural_model_name}}' => ucfirst(strtolower(Str::plural($controller_name))),
            '{{lc_model}}' => strtolower($controller_name),
            '{{uc_model}}' => ucfirst($controller_name),
            '{{uc_model_modular}}' => ucfirst(Str::plural($controller_name)),
    ...
    private function prepare_form_inputs(array $fields, array $types, $model_name, $slug, array $db_only, array $nullable)
    {
        ...
        foreach ($types as $key => $type) {
            ...
            $fieldTemplate = $this->replace_template($fieldTemplate, array(
                ...
                '{{lcf_field_plural}}' => Str::plural(str_replace('_id', '', $field_name)),
    ...
    private function create_admin_list_template($controller_name, $slug)
    {
        ...
        $AdminRoutesTemplate = $this->replace_template($AdminRoutesTemplate, array(
            ...
            '{{lcf_controller_name}}' => strtolower($controller_name),
            '{{lcf_plural_controller_name}}' => strtolower(Str::plural($controller_name))
        ));
